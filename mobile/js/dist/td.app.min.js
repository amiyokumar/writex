      ...
      function (
        ...
        $ionicNavBarDelegate.showBackButton(!0),
          ...
          ($scope.fetchTickets = function () {
            return (
              ...
              Tickets.all($scope.page)
                .then(
                  function (response) {
                    if (_.size(response.data.tickets) < 1) $scope.hasMoreTickets = !1;
                    else if (0 == $scope.page) $scope.tickets = response.data.tickets;
                    else {
                      var a = $scope.tickets;
                      0 < _.size(a) &&
                        ($scope.tickets = _.uniq(_.union(a, response.data.tickets), !1, function (i, k, a) {
                          return i._id;
                        }));
                    }
                  },
          ...
          ($scope.openNewTicket = function () {
            var groups = Groups.getHasAccess(),
              types = TicketTypes.all();
            $q.all([groups, types])
              .then(
                function (results) {
                  ($scope.groups = results[0].data.groups),
                    ($scope.ticketTypes = results[1].data.filter((type) => type.name === 'Sales')),
                    $scope.ticketTypes[0] &&
                      $scope.ticketTypes[0]._id &&
                      ($scope.selected.ticketType = $scope.ticketTypes[0]),
                    ($scope.modalNewTicketForm = { subject: '', issue: '' }),
                    $scope.groups[0] && $scope.groups[0]._id && ($scope.selected.group = $scope.groups[0]),
                    ($scope.currencies = ['AUD', 'USD', 'INR', 'EURO', 'POUND']), //TODO Make default currencies global
          ...
          ($scope.submitNewTicket = function ($event) {
            ...
            var ticket = {
              ...
              ticketID: this.modalNewTicketForm.ticketID,
              module: this.modalNewTicketForm.module,
              universityName: this.modalNewTicketForm.universityName,
              currency: $scope.selected.currency,
              invoiceAmount: this.modalNewTicketForm.invoiceAmount,
              invoiceNumber: this.modalNewTicketForm.invoiceNumber,
              wordCount: this.modalNewTicketForm.wordCount,
            };
            ticket.type &&
              ticket.subject &&
              ticket.issue &&
