...
const TicketFieldEnums = require('../settings/enums');
 ...
 * @property {Array} users An array of user _ids that will have read/write permission on the ticket.
 * @property {String} universityName
 * @property {String} module
 * @property {Number} wordCount ```Required``` Words for the assignment
 * @property {String} verdict
 * @property {Number} marks This field will be set when verdict is set. Will be used in dashboard
 * @property {String} workCategory Category field will be maintained by users. No extra collection creation for type checks
 * @property {String} currency
 * @property {Number} invoiceAmount ```Required``` Amount for this ticket
 * @property {Team} team ```Required``` Team this Ticket is under.
 * @property {Ticket} ticket Parent ticket of the ticket.
 * @property {Array} queries An array of {@link Query} items
 * @property {String} ticketID ```Required``` ```unique``` User enetered Ticket ID
 * @property {String} invoiceNumber ```Required``` Invoice Number
 */
const ticketSchema = mongoose.Schema({
  ...
  verdict: {
    type: String,
    enum: TicketFieldEnums.verdicts,
  },
  wordCount: {
    type: Number,
    required: true,
  ...
  workCategory: {
    type: String,
    enum: TicketFieldEnums.workCategory,
  },
  invoiceAmount: {
    type: Number,
    required: true,
  ...
  currency: {
    type: String,
    enum: TicketFieldEnums.currencies,
  },
  team: {
    type: mongoose.Schema.Types.ObjectId,
    required: false,
 ...
 * @param {function} callback Callback with the updated ticket.
 */
ticketSchema.methods.clearTeam = function (ownerId, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.team = undefined;
    const historyItem = {
      action: 'ticket:set:assignee',
      description: 'Team was cleared',
      owner: ownerId,
    };
 ...
 * Sets this ticket under the given group Id
 * @instance
 * @method setTicketTeam
 * @memberof Ticket
 *
 * @param {Object} ownerId Account ID preforming this action
 * @param {Object} teamId Team Id to assign this ticket to
 * @param {function} callback Callback with the updated ticket.
 */
ticketSchema.methods.setTicketTeam = function (ownerId, teamId, callback) {
  const self = this;
  self.team = teamId;

  self.populate('team', function (err, ticket) {
    if (err) return callback(err);

    const historyItem = {
      action: 'ticket:set:team',
      description: 'Ticket Team set to: ' + ticket.team.name,
      owner: ownerId,
    };
    self.history.push(historyItem);
 ...
 *
 * @param {Object} ownerId Account ID preforming this action
 * @param {Object} queryId Comment ID to update
 * @param {String} queryText Text to update the comment to
 * @param {function} callback Callback with the updated ticket.
 * @example
 * query.updateQuery({ownerId}, {queryId} 'This is the new query string.', function(err, t) {
 *    if (err) throw err;
 *
 *    query.save(function(err, t) {
 *       if (err) throw err;
 *    });
 * });
 */
ticketSchema.methods.updateQuery = function (ownerId, queryId, queryText, queryResolved, callback) {
  const self = this;
  return new Promise(async (resolve, reject) => {
    const query = await querySchema.getQueryById(queryId);

    if (!query) {
      if (typeof callback === 'function') callback('Invalid Query', null);
      return reject(new Error('Invalid Query'));
    }

    query.query = queryText;
    if (queryResolved) query.resolved = true;

    const historyItem = {
      action: 'ticket:query:updated',
      description: 'Query was updated: ' + queryId,
      owner: ownerId,
    };
    self.history.push(historyItem);
...
ticketSchema.methods.setUniversityName = function (ownerId, universityName, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.universityName = universityName;
    const historyItem = {
      action: 'ticket:update:universityName',
...
ticketSchema.methods.setModule = function (ownerId, module, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.module = module;
    const historyItem = {
      action: 'ticket:update:module',
...
ticketSchema.methods.setVerdict = function (ownerId, verdict, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.verdict = verdict;
    const historyItem = {
      action: 'ticket:update:verdict',
      description: 'Verdict updated.',
      owner: ownerId,
    };
...
ticketSchema.methods.addUsers = function (userId, callback) {
  const self = this;

  const hasUser = _.some(self.users, function (i) {
    return i._id.toString() === userId.toString();
  });
...
ticketSchema.methods.setMarks = function (ownerId, marks, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.marks = marks;
    const historyItem = {
      action: 'ticket:update:marks',
      description: 'Marks added.',
      owner: ownerId,
    };
...
ticketSchema.methods.setInvoiceAmount = function (ownerId, invoiceAmount, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.invoiceAmount = invoiceAmount;
    const historyItem = {
      action: 'ticket:update:invoiceAmount',
      description: 'Invoice Amount added.',
      owner: ownerId,
    };
...
ticketSchema.methods.setWorkCategory = function (ownerId, workCategory, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.workCategory = workCategory;
    const historyItem = {
      action: 'ticket:update:workCategory',
      description: 'Work Category added.',
      owner: ownerId,
    };
...
ticketSchema.methods.setWordCount = function (ownerId, wordCount, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.wordCount = wordCount;
    const historyItem = {
      action: 'ticket:update:wordCount',
      description: 'Word Count added.',
      owner: ownerId,
    };
...
ticketSchema.methods.setCurrency = function (ownerId, currency, callback) {
  const self = this;
  return new Promise((resolve) => {
    self.currency = currency;
    const historyItem = {
      action: 'ticket:update:currency',
...
ticketSchema.statics.getTickets = function (grpIds, callback) {
  ...
  const q = self
    ...
    .populate('type tags status group queries')
    .sort({ status: 1 });
...
function buildQueryWithObject(SELF, grpId, object, count) {
  ...
  let query;
  if (count)
    query = SELF.model(COLLECTION).countDocuments({
      groups: { $in: grpId },
      users: { $in: object.currentUser },
      deleted: false,
    });
  else {
    query = SELF.model(COLLECTION)
      .find({ group: { $in: grpId }, deleted: false, users: { $in: object.currentUser } })
      .populate(
  ...
  if (object.filter) {
    ...
    if (object.filter.date) {
      ...
      if (object.filter.date.start) {
        startDate = moment.tz(moment(object.filter.date.start, object.dateFormat), object.timezone).format();
      }
      if (object.filter.date.end) {
        endDate = moment.tz(moment(object.filter.date.end, object.dateFormat), object.timezone).format();
      }
      query.where({ date: { $gte: startDate, $lte: endDate } });

      if (object.filter.date.due) {
        endDate = moment.tz(moment(object.filter.date.due, object.dateFormat), object.timezone).format();
        query.where({ dueDate: { $gte: endDate } });
      }
    }
  }
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const formats = await settingSchema.getSettingsByName(['gen:timezone', 'gen:shortDateFormat']);
        formats.forEach((format) => {
          if (format.name === 'gen:timezone') object.timezone = format.value;
          else if (format.name === 'gen:shortDateFormat') object.dateFormat = format.value;
        });
...
ticketSchema.statics.getTicketByUid = function (uid, callback) {
  ...
  const q = self
    ...
    .populate('type tags status group team')
    .populate({ path: 'parent', populate: { path: 'type' } })
    .populate({
      path: 'queries',
      model: querySchema,
      populate: [
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      ...
      const q = self
        ...
        .populate('type tags status parent queries team')
        .populate({
          path: 'group',
          model: groupSchema,
          populate: [
 ...
 * @memberof Ticket
 * @static
 * @method getChildTicketsByUid
 *
 * @param {Number} uid Unique Id for ticket.
 * @param {function} callback MongoDB Query Callback
 */
ticketSchema.statics.getChildTicketsByUid = function (uid, callback) {
  if (_.isUndefined(uid)) return callback('Invalid Uid - TicketSchema.getChildTicketsByUid()', null);

  const self = this;

  const q = self
    .model(COLLECTION)
    .find({ parent: uid, deleted: false })
    .populate(
      'owner assignee comments.owner notes.owner subscribers history.owner',
      'username fullname email role image title'
    )
    .populate('type tags status group team parent');
 ...
 * Updates a given query
 * @instance
 * @method markQueryAsResolved
 * @memberof Ticket
 *
 * @param {Object} resolvedBy Account ID preforming this action
 * @param {Object} queryId Comment ID to update
 * @param {function} callback Callback with the updated ticket.
 * @example
 * query.setQueryAsResolved({ownerId}, {queryId} , function(err, t) {
 *    if (err) throw err;
 *
 *    query.save(function(err, t) {
 *       if (err) throw err;
 ...
 * });
 */
ticketSchema.methods.setQueryAsResolved = function (ownerId, queryId, callback) {
  const self = this;
  return new Promise(async (resolve, reject) => {
    const query = await querySchema.getQueryById(queryId);
    ...
    query.resolved = true;
    query.resolvedBy = ownerId;
    query.updated = Date.now();

    const historyItem = {
      action: 'ticket:query:resolved',
      description: 'Query was resolved: ' + queryId,
      owner: ownerId,
    };
    self.history.push(historyItem);
