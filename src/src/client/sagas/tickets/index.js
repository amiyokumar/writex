...
function* createChildTicket({ payload }) {
  try {
    const response = yield call(api.tickets.createChild, payload);
    const sessionUser = yield select(getSessionUser);
    yield put({ type: CREATE_CHILD_TICKET.SUCCESS, response, sessionUser });
    yield put({ type: HIDE_MODAL.ACTION });
  } catch (error) {
    const errorText = error.response.data.error;
    helpers.UI.showSnackbar(`Error: ${errorText}`, true);
    Log.error(errorText, error.response);
    yield put({ type: CREATE_CHILD_TICKET.ERROR, error });
  }
}
...
export default function* watcher() {
  ...
  yield takeLatest(CREATE_CHILD_TICKET.ACTION, createChildTicket);
