...
function getDatabaseVersion(callback) {
  SettingsSchema.getSettingByName('gen:version', function (err, setting) {
    ...
    if (!setting) {
      if (semver.satisfies('1.2.0', '>=1.0.11')) {
    ...
    [
      function (next) {
        getDatabaseVersion(function (err, dbVer) {
          ...
          if (semver.satisfies(databaseVersion, '<1.0.0')) {
            throw new Error('Please upgrade to v1.0.0 Exiting...');
          }
          return next();
        });
      },
      function (next) {
        if (semver.satisfies(semver.coerce(databaseVersion)?.version, '<1.0.0')) {
          async.parallel(
            [
              function (done) {
