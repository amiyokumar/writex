  ...
  Models.Ticket.getTicketByUid(uid, function (err, ticket) {
    ...
    if (req.user.role.isAdmin || req.user.role.isAgent) {
      Models.Department.getDepartmentGroupsOfUser(req.user._id, function (err, dbGroups) {
        ...
        if (groups.includes(ticket.group._id.toString()) && ticket.users.includes(req.user._id)) {
...
ticketsV2.childrens = async function (req, res) {
  const uid = req.params.uid;
  if (!uid) return apiUtils.sendApiError(res, 400, 'Invalid Parameters');

  Models.Ticket.getChildTicketsByUid(uid, function (err, tickets) {
    if (err) return apiUtils.sendApiError(res, 500, err);
    return apiUtils.sendApiSuccess(res, { tickets });
  });
};
ticketsV2.canDevTicketClose = function (req, res) {
  const parentId = req.params.parentid;
  if (!parentId) return apiUtils.sendApiError(res, 400, 'Invalid Parameters');
  Models.Ticket.getChildTicketsByUid(parentId, function (err, tickets) {
    if (err) return apiUtils.sendApiError(res, 500, err);
    const canClose = tickets.every((ticket) => {
      if (ticket.type.name === 'Quality' || ticket.type.name === 'Accounts') {
        return ticket.status.name === 'Closed';
      }
      return true;
    });
    return apiUtils.sendApiSuccess(res, { canClose });
  });
};

ticketsV2.canParentTicketClose = function (req, res) {
  const ticketId = req.params.id;
  if (!ticketId) return apiUtils.sendApiError(res, 400, 'Invalid Parameters');

  Models.Ticket.getChildTicketsByUid(ticketId, function (err, tickets) {
    if (err) return apiUtils.sendApiError(res, 500, err);
    const canClose = tickets.every((ticket) => ticket.status.name === 'Closed');
    return apiUtils.sendApiSuccess(res, { canClose });
  });
};
