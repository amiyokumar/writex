...
ticketsController.uploadAttachment = function (req, res) {
  ...
  busboy.on('file', function (name, file, info) {
    ...
    const savePath = path.join(__dirname, '../../public/uploads/tickets');
    let sanitizedFilename = object.ticketId + '_' + filename.replace(/[^a-z0-9.]/gi, '_').toLowerCase();
    ...
    events.push(function (cb) {
      ...
      objectHelper.uploadFile(object.filePath, object.filename);
    });

    file.pipe(fstream);
  });

  //objectHelper.uploadFile(savePath,sanitizedFilename)
  busboy.on('finish', function () {
    async.series(events, function () {
      ...
      ticketSchema.getTicketById(object.ticketId, function (err, ticket) {
        ...
        const attachment = {
          ...
          path: '/uploads/tickets/' + object.filename,
...
ticketsController.getAttachment = function (req, res, err) {
  let attachmentName = req.params.attachmentName;
  console.log('Downloading file: ', attachmentName);
  objectHelper.handleReadData(attachmentName, res);
};

// ticketsController.getAttachment = function (req, res, err) {
//   let attachmentName = req.params.attachmentName;
//   console.log('Downloading file: ', attachmentName);
//   minioClient.presignedGetObject(process.env.E2E_BUCKET_NAME, attachmentName, 24 * 60 * 60, (err, url) => {
//     if (err) {
//       console.error(err);
//       return;
//     }
