...
const objectHelper = require('../../../helpers/objectstore/index');
...
apiTickets.create = function (req, res) {
  ...
  const { ticketID, invoiceAmount, invoiceNumber } = postData;
  ...
  if (
    ...
    !postData.issue ||
    !postData.module ||
    !postData.universityName ||
    !postData.ticketID ||
    !postData.invoiceNumber ||
    !_.isNumber(_.toNumber(postData.wordCount)) ||
    !_.isNumber(_.toNumber(postData.invoiceAmount))
  )
  ...
  async.waterfall(
    [
      ...
      function (status, user, done) {
        var UserSchema = require('../../../models/user');
        UserSchema.find()
          .populate('role') //TODO filter not working
          .sort({ _id: 1 })
          .select('_id')
          .exec(function (err, users) {
            users = users.filter(function (user) {
              return user.role.normalized === 'admin' || user.role.normalized === 'pm';
            });
            done(err, users, status, user);
          });
      },
      function (adminAndManagers, status, user, done) {
        const TicketTypeSchema = require('../../../models/tickettype');
        TicketTypeSchema.find()
          .sort({ _id: 1 })
          .select('_id name')
          .exec(function (err, ticketTypes) {
            done(err, ticketTypes, adminAndManagers, status, user);
          });
      },
      function (ticketTypes, adminAndManagers, status, user, done) {
        ...
        postData.subject = sanitizeHtml(postData.subject).trim();
        postData.universityName = sanitizeHtml(postData.universityName).trim();
        postData.module = sanitizeHtml(postData.module).trim();
        postData.wordCount = _.toNumber(postData.wordCount);
        postData.module = sanitizeHtml(postData.module).trim();

        var marked = require('marked');
        var tIssue = postData.issue;
        tIssue = tIssue.replace(/(\r\n|\n\r|\r|\n)/g, '<br>');
        tIssue = sanitizeHtml(tIssue).trim();
        postData.issue = xss(marked.parse(tIssue));
        postData.history = [HistoryItem];
        postData.subscribers = [user._id];
        postData.users = _.includes(adminAndManagers, req.user._id)
          ? adminAndManagers
          : [...adminAndManagers, req.user._id];

        const salesTicketType = _.find(ticketTypes, { name: 'Sales' });
        postData.type = salesTicketType._id;
        postData.invoiceAmount = _.toNumber(postData.invoiceAmount);
        postData.ticketID = sanitizeHtml(postData.ticketID).trim().toUpperCase() + '-SAL';
        postData.invoiceNumber = '';
        ...
        ticket.save(function (err, t) {
          ...
          t.populate('group owner priority', function (err, tt) {
            ...
            postData.subscribers = adminAndManagers;
            postData.parent = tt._id;
            HistoryItem.description = `Ticket was created by parent ticket# ${tt.uid}`;
            postData.users = adminAndManagers;

            const chilTickets = [];
            ticketTypes.forEach((ticketType) => {
              switch (ticketType.name) {
                case 'Accounts':
                  postData.invoiceAmount = _.toNumber(invoiceAmount);
                  postData.type = ticketType._id;
                  postData.ticketID = ticketID + '-ACC';
                  postData.invoiceNumber = invoiceNumber;
                  chilTickets.push(new TicketSchema(postData));
                  break;
                case 'Quality':
                  postData.invoiceAmount = 0;
                  postData.type = ticketType._id;
                  postData.ticketID = ticketID + '-QA';
                  postData.invoiceNumber = '';
                  chilTickets.push(new TicketSchema(postData));
                  break;
                case 'Dev':
                  postData.invoiceAmount = 0;
                  postData.type = ticketType._id;
                  postData.ticketID = ticketID + '-DEV';
                  postData.invoiceNumber = '';
                  chilTickets.push(new TicketSchema(postData));
                  break;
                case 'Sales':
                ...
                default:
                  postData.invoiceNumber = '';
                  postData.ticketID = ticketID + '-' + ticketType.name.toUpperCase();
                  chilTickets.push(new TicketSchema(postData));
                  break;
              }
            });
...
/**
 * @api {post} /api/v1/tickets/createchild Create Child Ticket
 * @apiName createChildTicket
 * @apiDescription Creates a child ticket with the given post data.
 * @apiVersion 0.1.0
 * @apiGroup Ticket
 * @apiHeader {string} accesstoken The access token for the logged in user
 *
 ...
 *      -H "Content-Type: application/json"
 *      -H "accesstoken: {accesstoken}"
 *      -d "{type: \"{typeId}\", issue: \"{issue}\", prioirty: \"{prioirty}\"}"
 *      -l http://localhost/api/v1/tickets/createchild
 *
 * @apiSuccess {boolean} success If the Request was a success
 * @apiSuccess {object} error Error, if occurred
 * @apiSuccess {object} ticket Saved Ticket Object
...
apiTickets.createChild = function (req, res) {
  var response = {};
  response.success = true;
  ...
  async.waterfall(
    [
      ...
      function (status, user, done) {
        var TicketSchema = require('../../../models/ticket');
        TicketSchema.findOne({ parent: postData.parentId, type: postData.type })
          .populate({ path: 'users', populate: { path: 'role' } })
          .select(
            '-_id -uid -__v -history -owner -date -updated -deleted -status -closedDate -dueDate -comments -notes'
          )
          .exec(function (err, ticket) {
            done(err, ticket, status, user);
          });
      },
      function (ticket, status, user, done) {
        if (user.deleted) return done({ status: 400, error: 'Invalid User' });
        if (!ticket) return done({ status: 400, error: 'Invalid Ticket Type Selected ' });
        if (ticket.ticketID === postData.ticketID) return done({ status: 400, error: 'Ticket ID must be unique' });

        var HistoryItem = {
          action: 'ticket:created',
          description: `Ticket was created from parent ticket# ${postData.parentTicketID}`,
          owner: req.user._id,
        };
        postData.history = [HistoryItem];
        postData.status = status._id;
        ...
        postData.group = ticket.group;
        postData.tags = ticket.tags;
        postData.subject = ticket.subject;
        postData.issue = ticket.issue;
        postData.subscribers = ticket.subscribers;
        postData.module = ticket.module;
        postData.universityName = ticket.universityName;
        postData.wordCount = ticket.wordCount;
        postData.workCategory = ticket.workCategory;
        postData.invoiceAmount = ticket.invoiceAmount;
        postData.currency = ticket.currency;
        postData.parent = ticket.parent;
        postData.invoiceNumber = ticket.invoiceNumber;
        postData.priority = ticket.priority;

        let userAccess = ticket.users;
        _.remove(userAccess, (user) => ['user', 'qa', 'dev', 'accounts', 'sales'].includes(user.role.normalized));
        postData.users = userAccess;

        var TicketSchema = require('../../../models/ticket');
        var newTicket = new TicketSchema(postData);

        newTicket.save(function (err, t) {
          if (err) return done({ status: 400, error: err });

          t.populate('group owner priority parent', function (err, tt) {
            if (err) return done({ status: 400, error: err });
  ...
  ticketModel.getTicketById(ticketId, function (err, ticket) {
    ...
    ticket.getAttachment(attachmentId, function (a) {
      ticket.removeAttachment(user._id, attachmentId, function (err, ticket) {
        ...
        objectHelper.removeFile(a.name, (err) => res.status(500).json({ error: 'Unable to remove attachment.' }));
        var fs = require('fs');
...
apiTickets.subscribe = function (req, res) {
  ...
  if (data.user.toString() !== req.user._id.toString()) return res.status(401).json({ error: 'Un Ownerized!' });
  ...
  ticketModel.getTicketById(ticketId, function (err, ticket) {
    ...
    async.series(
      [
        function (callback) {
          require('../../../models/user').find({ _id: data.user }, function (err, user) {
            ...
            if (!user) return callback(new Error('Un Ownerized!'));
      ...
      function (err) {
        if (err) {
          winston.warn(err);
          return res.status(401).json({ error: 'Un Ownerized!' });
        }
...
/**
 * @api {post} /api/v1/tickets/addquery Add Query
 * @apiName addQuery
 * @apiDescription Adds query to the given Ticket Id
 * @apiVersion 0.1.0
 * @apiGroup Ticket
 * @apiHeader {string} accesstoken The access token for the logged in user
 * @apiExample Example usage:
 ...
 *      -H "Content-Type: application/json"
 *      -H "accesstoken: {accesstoken}"
 *      -d "{\"query\":\"{query}\",\"owner\":{ownerId}, ticketId: \"{ticketId}\"}"
 *      -l http://localhost/api/v1/tickets/addquery
 *
 * @apiParamExample {json} Request:
 * {
 *      "query": "Query Text",
 ...
 */
apiTickets.postQuery = async function (req, res) {
  var queryJSON = req.body;
  var query = queryJSON.query;
  var owner = queryJSON.ownerId || req.user._id;
  var ticketId = queryJSON._id;

  if (_.isUndefined(ticketId)) return res.status(400).json({ success: false, error: 'Invalid Post Data' });
  if (_.isUndefined(query)) return res.status(400).json({ success: false, error: 'Invalid Post Data' });

  const ticketModel = require('../../../models/ticket');
  try {
    const ticket = await ticketModel.getTicketById(ticketId);
    if (_.isUndefined(ticket)) return res.status(400).json({ success: false, error: 'Invalid Post Data' });

    var marked = require('marked');
    marked.setOptions({
      breaks: true,
    });
    query = sanitizeHtml(query).trim();
    var Query = {
      owner: owner,
      date: new Date(),
      updated: new Date(),
      query: xss(marked.parse(query)),
    };

    const querySchema = require('../../../models/query');
    const newQuery = await new querySchema(Query).save();

    ticket.updated = Date.now();
    ticket.queries.push(newQuery._id);
    var HistoryItem = {
      action: 'ticket:query:added',
      description: 'Query was added',
      owner: owner,
    };
    ticket.history.push(HistoryItem);
    //Push the query to parent ticket
    if (!_.isUndefined(ticket.parent)) {
      ticket.parent.queries.push(newQuery._id);
      // ticket.parent.history.push(HistoryItem);
      await ticket.parent.save();
    }
    ...
    emitter.emit('ticket:comment:added', ticket, Query, req.headers.host); //TODO
