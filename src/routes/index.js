...
// const swaggerRoute = require('./docs');
...
function mainRoutes(router, middleware, controllers) {
  ...
  router.get(
    '/tickets/getattachment/:attachmentName',
    middleware.redirectToLogin,
    middleware.csrfCheck,
    controllers.tickets.getAttachment
  );
  ...
  router.post('/api/v1/admin/restart', middleware.csrfCheck, middleware.api, middleware.isAdmin, function (req, res) {
    ...
    pm2.connect(function (err) {
      ...
      pm2.restart('writex', function (err) {
        if (err) {
          res.status(400).send(err);
  ...
  if (global.env === 'development') {
    ...
    // router.get('/api/v1/docs', middleware.redirectToLogin, swaggerRoute);
  }
}
